...
class NetflixMirrorProvider implements StreamingProvider {
    ...
    while (retries < 5) {
      try {
        final response = await http
            .post(Uri.parse('$_baseUrl/tv/p.php'))
  ...
  Future<Map<String, List<Movie>>> getHomePage() async {
    await bypass();
    final response = await http.get(Uri.parse('$_baseUrl/tv/home'), headers: _apiHeaders);
  ...
  Future<NetflixMovieDetails> getMovieDetails(Movie movie) async {
    ...
    if (response.statusCode == 200) {
      ...
      if (postData.episodes.isEmpty || postData.episodes.first == null) {
        episodes.add(NetflixEpisode(id: movie.id.toString(), title: postData.title, season: '1', episode: '1'));
      } else {
        for (var episode in postData.episodes) {
          if (episode != null) {
            episodes.add(NetflixEpisode(
              id: episode.id,
              title: episode.t,
              season: episode.s.replaceAll('S', ''),
        ...
        for (var source in item.sources) {
          ...
          try {
            ...
            if (kDebugMode) {
              ...
              print('M3U8 Response Body: ${m3u8Response.body}');
            }
          } catch (e) {
            if (kDebugMode) {
