...
class MovieApiService {
  ...
  Future<TvShow> getTvShowDetails(int tvShowId) async {
    final response = await http.get(
      Uri.parse(
        '${ApiConstants.baseUrl}/tv/$tvShowId?api_key=${ApiConstants.apiKey}',
      ),
    );
    if (response.statusCode == 200) {
      final data = json.decode(response.body);
  ...
  Future<Season> getSeasonDetails(int tvShowId, int seasonNumber) async {
    final response = await http.get(
      Uri.parse(
        '${ApiConstants.baseUrl}/tv/$tvShowId/season/$seasonNumber?api_key=${ApiConstants.apiKey}',
      ),
    );
    if (response.statusCode == 200) {
      final data = json.decode(response.body);
  ...
  Future<List<SearchResult>> searchMulti(String query) async {
    final response = await http.get(
      Uri.parse(
        '${ApiConstants.baseUrl}/search/multi?api_key=${ApiConstants.apiKey}&query=$query',
      ),
    );
    if (response.statusCode == 200) {
      final data = json.decode(response.body);
